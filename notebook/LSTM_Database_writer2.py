...
class DatabaseWriter:
    ...
    def write_simple_results(self, results: pd.DataFrame, if_exists: str = 'append') -> bool:
        """
        Write results to simple anomaly_detection_results table
        (Compatible with the format from LSTM_Test_Database_integration1.py)
        
        Args:
            results: DataFrame with prediction results
            if_exists: 'replace' or 'append' (default: 'append')
        ...
        try:
            ...
            # Handle timestamp columns - rename to avoid SQL reserved keywords
            if 'datetime' in df.columns:
                df['event_timestamp'] = pd.to_datetime(df['datetime'])
            elif 'end_datetime' in df.columns:
                df['event_timestamp'] = pd.to_datetime(df['end_datetime'])
            
            if 'start_datetime' in df.columns:
                df['event_timestamp_start'] = pd.to_datetime(df['start_datetime'])
            ...
            # Drop the table if replace mode
            if if_exists == 'replace':
                try:
                    cursor.execute("DROP TABLE IF EXISTS dbo.anomaly_detection_results")
                    logger.info("✓ Dropped existing table 'anomaly_detection_results'")
                except:
                    pass
            
            # Prepare data for insertion
            table_name = "dbo.anomaly_detection_results"
            
            # Select key columns for the simple table
            simple_cols = []
            col_mapping = {
                ...
                'event_timestamp_start': 'event_timestamp_start',
                'compressor_id': 'compressor_id',
                'analysis_timestamp': 'analysis_timestamp'
            }
            
            # Build column list and data
            available_cols = [col for col in col_mapping.keys() if col in df.columns]
            
            # Add feature error columns
            feature_error_cols = [col for col in df.columns if col.startswith('error_')]
            available_cols.extend(feature_error_cols)
            
            df_simple = df[available_cols].copy()
            
            logger.info(f"Writing {len(df_simple)} rows to {table_name}...")
            logger.info(f"Columns: {list(df_simple.columns)}")
            
            # Create table if doesn't exist
            create_table_sql = """
                IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'anomaly_detection_results')
                BEGIN
                    CREATE TABLE dbo.anomaly_detection_results (
                        id INT IDENTITY(1,1) PRIMARY KEY,
                        sequence_index INT,
                        reconstruction_error FLOAT,
                        threshold FLOAT,
                        is_anomaly_point INT,
                        triggered_anomaly INT,
                        anomaly_score FLOAT,
                        consecutive_count INT,
                        event_timestamp DATETIME2,
                        event_timestamp_start DATETIME2,
                        compressor_id NVARCHAR(100),
                        analysis_timestamp DATETIME2,
                        error_filter_dp FLOAT,
                        error_seal_gas_flow FLOAT,
                        error_seal_gas_diff_pressure FLOAT,
                        error_seal_gas_temp FLOAT,
                        error_primary_vent_flow FLOAT,
                        error_primary_vent_pressure FLOAT,
                        error_secondary_seal_gas_flow FLOAT,
                        error_separation_seal_gas_flow FLOAT,
                        error_separation_seal_gas_pressure FLOAT,
                        error_seal_gas_to_vent_diff_pressure FLOAT,
                        error_encoding FLOAT
                    )
                END
            """
            cursor.execute(create_table_sql)
            logger.info("✓ Table structure verified/created")
            ...
            logger.info(f"✓ Successfully wrote {len(data_tuples)} rows to {table_name}")
            
            return True
            
        except Exception as e:
            logger.error(f"✗ Error writing simple results: {str(e)}")
            if self.connection:
                self.connection.rollback()
            return False
